...
//    @CircuitBreaker(name = "protocolService", fallbackMethod = "protocolFallback")
    ...
    public ConsentResponseDTO createConsent(ConsentRequestDTO dto) throws Exception {
        ...
        ConsentResponseDTO response = ConsentMapper.toResponse(saved);

        // Record in provenance table
        Map<String, Object> map = Map.of("consentMethod", saved.getConsentMethod(),
                "status", saved.getStatus(),
                "participantId", saved.getParticipantId(),
                "protocolId", saved.getProtocolId());ObjectMapper mapper = new ObjectMapper();
        ProvenanceRequestDTO requestDTO = new ProvenanceRequestDTO("CREATE_CONSENT", "consent_record", UserUtil.getCurrentUserId(), saved.getConsentId(), mapper.writeValueAsString(map));
    ...
    public String withdrawConsent(ConsentWithdrawalDTO dto) {
        ...
        //Record
        Map<String, Object> map = Map.of("consentId", saved.getConsentId(),
                "withdrawnBy", saved.getWithdrawnBy()
        );
        ObjectMapper mapper = new ObjectMapper();
        ProvenanceRequestDTO requestDTO = new ProvenanceRequestDTO("WITHDRAW_CONSENT", "consent_withdrawal", UserUtil.getCurrentUserId(), saved.getWithdrawalId(), mapper.writeValueAsString(map));
        provenanceClient.recordProvenanceData(requestDTO);
